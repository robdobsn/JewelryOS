...
BUILD_CONFIG_DIR = buildConfigs/$(BUILD_CONFIG_NAME)
...
flash: build
	$(PYTHON_FOR_FLASH_MONITOR) $(BUILD_DIR)/_deps/raftcore-src/scripts/flashUsingPartitionCSV.py $(BUILD_RAFT_ARTEFACTS_DIR)/partitions.csv $(BUILD_DIR) $(BUILD_CONFIG_NAME).bin $(PORT) -s $(BUILD_CONFIG_DIR)/sdkconfig
